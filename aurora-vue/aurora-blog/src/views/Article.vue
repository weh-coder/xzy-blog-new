      ...
      <div class="post-header">
        ...
        <div class="flex flex-row items-center justify-start mt-8 mb-4">
          ...
          <div class="post-stats" v-if="wordNum !== '' && readTime !== ''">
            ...
            <span>
              <svg style="width: 1.5em;height:1.5em;display:inline-block; stroke: rgba(255, 255, 255, var(--tw-text-opacity));" class="svg-icon" aria-hidden="true" data-v-c8a70580="" data-v-7b23b1e6=""><use href="#icon-message" data-v-c8a70580=""></use></svg>
              <span class="pl-2 opacity-70">
                {{commentNum}}
              </span>
            </span>
...
<script lang="ts">
import { Sidebar, Profile, Navigator, RecentComment } from '@/components/Sidebar'
import {
  computed,
  defineComponent,
  nextTick,
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchComments = () => {
      ...
      api.getComments(params).then(({ data }) => {
        if(data.data.count!=null){
          reactiveData.commentNum=data.data.count
          for(let i=0;i<data.data.count;i++){
            if(data.data.records[i].replyDTOs!=null){
              reactiveData.commentNum+=data.data.records[i].replyDTOs.length;
            }
          }
        }else{
          reactiveData.commentNum=0;
        }
    ...
    return {
      ...
      formatTime,
      commentNum:computed(() => {
        return reactiveData.commentNum;
      }),
      t
    }
  }
