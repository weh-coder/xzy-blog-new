<template>
  ...
  <el-dialog v-model="loginDialogVisible" width="30%" :fullscreen="isMobile">
    <el-form @keyup.enter.native="login">
      <el-form-item model="userInfo" class="mt-5">
        <el-input v-model="loginInfo.username" :placeholder="t('settings.mail—address')" />
      </el-form-item>
      <el-form-item model="userInfo" type="password" class="mt-8">
        <el-input v-model="loginInfo.password" type="password" show-password :placeholder="t('settings.password')" />
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="login" size="large" class="mx-auto mt-3">{{ t('settings.sign-in')
          }}</el-button>
      </el-form-item>
      <center v-if="qqLoginstatu === 1">
        <small>{{ t('settings.other-login') }}</small>
        <el-form-item>
          <svg class="svg-icon  mx-auto qq-sign" aria-hidden="true" @click="qqLogin" data-v-c8a70580=""
            data-v-7ccb3f1d="">
            <use href="#icon-qq" data-v-c8a70580=""></use>
          </svg>
        </el-form-item>
      </center>
      <div class="mt-8">
        <span class="text" @click="openRegisterDialog">{{ t('settings.create-account') }}</span>
        <span class="text float-right" @click="openForgetPasswordDialog">{{ t('settings.forgot-password') }}</span>
      </div>
    </el-form>
  </el-dialog>
  <el-dialog v-model="registerDialogVisible" width="30%" :fullscreen="isMobile">
    <el-form>
      ...
      <el-form-item model="userInfo" class="mt-8">
        <el-input v-model="loginInfo.code" :placeholder="t('settings.enter-code')">
          <template #append>
      ...
      <el-form-item>
        <el-button type="primary" @click="register" size="large" class="mx-auto mt-3">{{ t('settings.register-account')
          }}</el-button>
      </el-form-item>
      <span class="text" @click="returnLoginDialog">{{ t('settings.have-account') }}</span>
    </el-form>
  </el-dialog>
  <el-dialog v-model="forgetPasswordDialogVisible" width="30%" :fullscreen="isMobile">
    <el-form>
      ...
      <el-form-item>
        <el-button type="primary" @click="updatePassword" size="large" class="mx-auto mt-3">{{ t('settings.confirm')
          }}</el-button>
      </el-form-item>
      <span class="text" @click="returnLoginDialog">{{ t('settings.return-to-login') }}</span>
    </el-form>
  </el-dialog>
  <el-dialog v-model="articlePasswordDialogVisible" width="30%" :fullscreen="isMobile">
    <el-form @submit.native.prevent @keyup.enter.native="accessArticle">
      <el-form-item model="userInfo" class="mt-5">
        <el-input id="article-password-input" v-model="articlePassword"
          :placeholder="t('settings.password-protected')" />
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="accessArticle" size="large" class="mx-auto mt-3">{{
    t('settings.verify-password')}}</el-button>
      </el-form-item>
    </el-form>
  </el-dialog>
  <teleport to="body">
