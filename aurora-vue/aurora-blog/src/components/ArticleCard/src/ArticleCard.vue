        ...
        <div class="article-footer" v-if="article.author && article.createTime">
          <div class="flex flex-row items-center">
            ...
            <span class="text-ob-dim">
              ...
              <svg-icon icon-class="text-outline" style="stroke: gray" />
              <span class="opacity-70">
                {{ wordNum(article.articleContent) }}
              </span>
            </span>
          </div>
        </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const wordNum =(content: any)=>{
      let num = Math.round(deleteHTMLTag(content).length / 100) / 10;

      return num <0.1 ? '⬇100' : num+'k';
    }
    return {
